...
| v0.3 | M2 快速預習 | 預覽卡、發音、文章短句、三分類 | session 可續做；單字可回到來源情境；鍵盤與手機可操作 |
...
M0、M1 已完成。M1A 已建立文章擷取、候選排序、冪等工作與自動補全 adapter；先修正目前線上執行發現的 Apps Script 全域載入錯誤，再完成 schema v4 遷移、API 金鑰設定與 smoke test。之後進入 M2，優先實作 Parroto 評估中已採用的「來源短句、句中選字、播放發音、錯誤回流」最小閉環。
