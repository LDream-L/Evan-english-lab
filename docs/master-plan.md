...
| 情境聽說參考 | 參考 Parroto 的「真實內容→聽寫／跟讀→點字收錄→SRS→錯誤回流」閉環 | 先強化個人單字與短句學習；真人配對、排行榜、完整模考不提前納入 Vocabulary V1 |
...
- 26 秒概念的快速預習流程。
- EN→ZH、ZH→EN、拼字三種卡片；短句聽寫題型保留 schema，是否納入 v1.0 由 M3 驗收決定。
...
- 多人帳號、社群、排行榜、付款。
- 完整 Grammar 課程、長篇 Listening／Reading 課程與完整模擬考；Vocabulary V1 僅允許服務單字記憶的短句播放、跟讀與聽寫驗證。
- 抓取或複製刷刷庫及其他商業題庫內容。
...
| v0.3 | M2 快速預習 | 預覽卡、發音、文章短句、三分類、騎乘音訊 | session 可續做；單字可回到來源情境；35 分鐘純音訊清單不要求途中操作且不誤改 FSRS |
| v0.4 | M3 提取複習 | 三個核心題型、可選短句聽寫、rating、response time、review log | 重送不重複寫入；歷史與 state 一致；錯題可立刻重現並回流弱項 |
...
- 有文章來源時顯示必要短句與來源連結，允許從句中選字加入單字庫；不得保存或重製完整受著作權保護內容。
- 三分類：「已經會／有印象／不會」只決定初始學習路徑，不直接等同最終 FSRS rating。
- 發音 V1 優先瀏覽器 Speech Synthesis；聲音品質與可用性列入驗收。
- 加入 [M2R 騎乘音訊模式](m2r-riding-audio.md)：依 Evan 約 37 分鐘的機車通勤，預設建立 35 分鐘、出發前一次啟動、途中零作答的英文字義與例句播放清單。
- 騎乘音訊只記錄 `audio_exposure`，不直接更新答對率、mastery 或 FSRS；停車後才進入需要操作的正式複習。
- 第一版使用裝置端 Speech Synthesis 與 Screen Wake Lock，不承諾鎖屏／背景播放；Android 與 iPhone 實機驗收後，再決定是否導入預先生成音訊、PWA 或原生 App。

### M3

- 建立 card 狀態機：new、learning、review、relearning、suspended。
- 題型：EN→ZH 可自評；ZH→EN 與拼字可正規化後自動比對並允許人工覆核。
- 以 Parroto 的錯誤回流概念加入「錯字／聽錯片段→弱項佇列」；短句聽寫先以正規化文字比對，不以生成式 AI 直接判分。
...
完整評估與採用邊界見 [product-reference-review.md](product-reference-review.md)。目前結論如下：

| 參考產品 | 借鑑重點 | 現階段不採用 |
|---|---|---|
| 刷刷庫 | 碎片化刷題、逐題詳解、自動錯題本、弱項分類、文法單元、進度與分數落點 | 題目、詳解、介面資產與任何商業資料 |
| Parroto | 真實短內容、聽寫、跟讀、情境點字、SRS、錯誤複習、短時學習單元 | 真人配對、社群、排行榜、獎勵兌換、Pro 牌組、完整 IELTS／TOEIC 模考 |

Parroto 官方目前列出的功能包含聽寫、跟讀、AI 發音回饋、情境單字與間隔重複、短課程、遊戲、真人語音配對及 IELTS／TOEIC 模考。這些是產品功能事實或官方宣稱，不等於已由本專案獨立驗證其學習成效；「30 天改善」等成效數字不作為開發決策依據。

採用原則是保留學習閉環，不複製內容或外觀：文章候選字要能回到原句；預習與複習都共用同一份 `word/card/memory_state`；拼錯或聽錯會回流弱項；所有音訊、逐字稿與題目均須自建、使用可合法利用的來源，或只保存必要摘錄與來源連結。
...
```text
Vocabulary V1
→ Context Listening／Dictation V1
→ Shadowing／Pronunciation V1
→ Grammar V1
...
刷刷庫補足題型、文法與錯題路徑；Parroto 補足情境聽寫、跟讀與單字回流路徑。Evan TOEIC Lab 的差異是兩條路徑共用同一套可追溯單字、卡片、錯誤與 FSRS 記憶狀態，而不是各自形成孤島。
...
| 影片／音訊與逐字稿權利 | 自建、合法授權或只保存必要摘錄與來源連結；Parroto 只作流程參考 | 未確認授權不得鏡像課程、音訊或完整逐字稿 |
| 發音評分誤判與語音隱私 | Vocabulary V1 不把 AI 發音分數當唯一判定；錄音預設不長期保存 | 要導入雲端語音辨識前，先決定供應商、資料去向、保留期限與成本上限 |
| 騎乘音訊受瀏覽器背景限制 | 第一版出發前載入、保持頁面啟用且全程零操作；播放接觸不算正式作答 | 若實機要求鎖屏、媒體鍵或穩定背景播放，再評估音訊檔、PWA／原生 App 與 TTS 成本 |

## 15. 必須由 Evan 完成或核准的事項
...
4. M2R 騎乘音訊以實際手機、瀏覽器及安全帽音訊設備驗收背景／中斷行為。
5. M4 desired retention 與積壓門檻的產品選擇；未決前使用 0.90 與 20 的明示預設值。
6. v1.0 正式發布核准。
7. 網站全自動補全或 M2R 背景音訊若要使用付費 API，核准供應商與每月成本上限；在此之前不得啟用付費服務。

## 16. 下一個施工批次

M0、M1 已完成。M1A 已建立文章擷取、候選排序、冪等工作與自動補全 adapter；先修正目前線上執行發現的 Apps Script 全域載入錯誤，再完成 schema v4 遷移、API 金鑰設定與 smoke test。之後進入 M2，優先實作 Parroto 評估中已採用的「來源短句、句中選字、播放發音、錯誤回流」最小閉環，並交付 M2R-A 的 35 分鐘騎乘音訊 MVP。
