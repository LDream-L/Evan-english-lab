...
| M0 專案骨架 | 已完成 | 100% | 無 |
| M1 單字資料庫 | 已完成 | 100% | 無 |
| M1A 文章擷取／自動補全 | 擷取可用、AI 補全暫緩 | 90% | Gemini API 金鑰暫不啟用；不阻擋 M2 |
| M2R 騎乘音訊 Preview | 暫停使用，手機實測雙路無聲 | 70% | 需有獨立音檔來源及手機實機驗收 |
| M2 快速預習 | 文字預習第一版待線上驗收 | 40% | 發音、句中選字與正式複習仍未完成 |
| M3 正式複習 | 未開始 | 0% | 等 M2 |
| M4 自適應排程 | 未開始 | 0% | 等 M3 |
...
- 2026-09-25：正式資料庫加入 112 個自編 TOEIC 情境單字及 112 個人工例句，均 active、去重且來源關聯完整；全程不使用 Gemini。
- 2026-09-25：M2R 音訊修正已部署為第 6 版 `v0.2.3-m2r-audio`，沿用私人 Web App 原網址與「只有我自己」權限；新增停妥後試聽、實際起播狀態、8 秒未起播提示及語音錯誤訊息。線上驗證 112 字可建立 35 分鐘／177 次播放清單；雲端瀏覽器試聽回報 `synthesis-failed`。
- 2026-09-25：第 7 版 `v0.2.4-m2r-audio-test` 已更新原私人網址，新增不依賴文字轉語音的短提示音測試，並移除英文試聽前不必要的語音取消；三個雲端檔案重載後與 GitHub blob 完全一致。正式網站顯示版本、schema 正常、112 個啟用字、35 分鐘／177 次播放清單。雲端瀏覽器無法確認 iPhone 喇叭是否實際有聲，待實機分別測試提示音及英文語音。
- 2026-09-25：Evan 實測第 7 版「提示音」與「英文試聽」皆無聲，故撤回騎乘音訊可用的判定。M2 先推出不依賴 Gemini／音訊的 10 字文字預習：意思、詞性、原創例句與既有來源短句；三分類只記錄預習進度，可續做，不寫入 `reviews` 或 `memory_state`。音訊控制已停用，重新開放的條件為獨立音檔方案及手機實際有聲。

## 下一批

- 音訊改採經授權／可合法使用的獨立錄音或語音檔，並在 iPhone 的原生音訊播放器驗證；目前無可直接使用的音源，不再要求重試同一網頁語音流程。
- 以目前 112 個人工編輯且有例句的啟用單字進入 M2 快速預習；M1A 的 Gemini 補全維持暫緩。
